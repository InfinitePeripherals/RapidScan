...
namespace BLETest
{
    ...
    public class BLEServer
    {
        ...
        async void BarcodeIn(GattLocalCharacteristic sender, GattWriteRequestedEventArgs args)
        {
            ...
            // Check if the data correctly contains three elements
            if (elements.Length == 3 || elements.Length == 4)
            {
                // The middle element, which is the barcode, is at index 1
                string barcode = elements[1];
                ...
                SendBarcodeAsKeystrokes(barcode);
            }
        }
        //---------------------------------------------------------------------------------------------------------------------------------------
        private void SendBarcodeAsKeystrokes(string barcode)
        {
            // Inject the barcode as keystrokes
            inputSimulator.Keyboard.TextEntry(barcode);

            // Check the toggle state and conditionally insert a return
        ...
        private string GetConnectedRISLCard(string deviceID)
        {
            ...
            risl.SetFont(28, false).TextCenter(5, "BLE Server");
